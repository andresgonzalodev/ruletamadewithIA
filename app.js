...
const LSKEY = "ruleta_gamer_v5"; // nueva key para evitar arrastrar estados
...
function dedup(arr){ const s=new Set(); return arr.filter(x=>{const k=x.toLowerCase(); if(s.has(k))return false; s.add(k); return true;}); }
...
function drawWheel(highlightIdx=-1){
  ...
  for (let i=0;i<items.length;i++){
    ...
    ctx.fillStyle = colorFor(i, items.length); ctx.fill();

    ctx.strokeStyle="rgba(0,0,0,.25)"; ctx.lineWidth=1; ctx.stroke();

    const mode = elLabelMode?.value || "none";
    if (mode!=="none"){
      const mid=(start+end)/2, rText=outer*0.80;
      let text=""; if (mode==="index") text=String(i+1); if (mode==="abbr") text=abbrFromName(items[i],4);
      if (text){
        ctx.save();
        ctx.translate(cx,cy); ctx.rotate(mid);
        ctx.textAlign="center"; ctx.textBaseline="middle";
        ctx.font=`${Math.max(10, Math.floor(outer*0.06))}px ui-monospace, monospace`;
        ctx.fillStyle="rgba(0,0,0,.45)"; ctx.fillText(text, rText, 1.5);
        ctx.fillStyle="#e9eef7"; ctx.fillText(text, rText, 0);
        ctx.restore();
      }
    }
  }
  ...
  // hueco
  ctx.beginPath(); ctx.arc(cx,cy,inner,0,TAU); ctx.fillStyle="#0d111a"; ctx.fill();
  ...
  if (highlightIdx>=0){
    ...
    const end   = start + step;
    ctx.save();
    ctx.beginPath(); ctx.moveTo(cx,cy); ctx.arc(cx,cy,outer*1.04,start,end); ctx.closePath();
...
function animateWinnerZoom(idx){
  ...
  function frame(now){
    ...
    const u=Math.min(1,(now-t0)/dur), e=easeOutBack(u);

    drawWheel(idx);

    const grow=1+0.14*e, glow=Math.floor(6+10*e);
    const rOuter=outer*grow, rInner=inner*Math.max(0.85,1-0.15*e);

    ctx.save();
    ctx.beginPath(); ctx.moveTo(cx,cy); ctx.arc(cx,cy,rOuter,start,end); ctx.closePath();
    ctx.fillStyle="rgba(125,211,252,0.24)"; ctx.fill();

    ctx.strokeStyle="rgba(125,211,252,0.55)"; ctx.lineWidth=glow; ctx.stroke();

    const grad=ctx.createRadialGradient(cx,cy,rInner*0.2,cx,cy,rInner*1.05);
    grad.addColorStop(0,"rgba(125,211,252,0.25)");
    grad.addColorStop(1,"rgba(125,211,252,0)");
    ctx.fillStyle=grad; ctx.beginPath(); ctx.arc(cx,cy,rInner*1.05,0,TAU); ctx.fill();
    ctx.restore();
...
tickSound.preload='auto'; tickSound.volume=0.35;
function playTick(){
  const click = tickSound.cloneNode(true);
  click.currentTime = 0.15; click.volume = tickSound.volume;
  const p = click.play(); if (p && p.catch) p.catch(()=>{});
  setTimeout(()=>{ try{click.pause();}catch{} click.src=""; }, 140);
}
...
function stopAndAnnounce(){
  ...
  elWinner.classList.remove('pop'); void elWinner.offsetWidth; elWinner.classList.add('pop');

  elLast.textContent = `Ganador: ${name}`;
  if (name && !historyWinners.includes(name.toLowerCase())){
    historyWinners.push(name.toLowerCase()); saveState();
  }
...
function spin(){
  ...
  // cancelar overlays previos
  zoomActive=false; if (zoomRAF) { cancelAnimationFrame(zoomRAF); zoomRAF=null; }
  if (spinRAF) { cancelAnimationFrame(spinRAF); spinRAF=null; }

  elWinner.textContent="…"; elWinner.classList.remove('pop');

  // pool (exclusión lógica, la física manda el stop)
  ...
  // wobble params
  let wobblePhase=false, wobbleStart=0, baseAngle=0, wobbleAmp0=0;
  ...
  const WOBBLE_MIN_PX = 70; // bien visible

  function loop(now){
    const dt=Math.min(0.05,(now-last)/1000); last=now;

    if (!wobblePhase){
      const t=(now-t0)/1000, u=Math.min(1,t/T);
      ...
      if (u>=1){
        wobblePhase=true; wobbleStart=now; baseAngle=angle;

        // amplitud base por pixeles -> rad, y límite por tamaño de sector
        const maxAmpByPixels = px2rad(WOBBLE_MIN_PX);
        let amp = Math.min(maxAmpByPixels, stepAng*0.44);

        // no cruzar bordes del sector actual
        ...
        const margin = Math.max(px2rad(3), stepAng*0.06);
        const safeDist = Math.max(stepAng*0.5 - Math.abs(posInSector - stepAng*0.5) - margin, 0);
        wobbleAmp0 = Math.max(0, Math.min(amp, safeDist));

        if (wobbleAmp0 < 1e-4){
          spinning=false; drawWheel(); stopAndAnnounce(); return;
        }
      }else{
    ...
    // wobble amortiguado
    const tw=(now - wobbleStart)/WOBBLE_DUR;
    if (tw>=1){
      spinning=false; angle=baseAngle; drawWheel(); stopAndAnnounce(); return;
    }

    const amp = wobbleAmp0 * Math.exp(-0.85*tw);
...
elCanvas.addEventListener("mousemove",(e)=>{
  ...
  if (r<inner || r>outer){ elTip.hidden=true; elTip.classList.remove('show'); return; }
...
elCanvas.addEventListener("mouseleave",()=>{ if (!elTip) return; elTip.classList.remove('show'); elTip.hidden=true; });
...
(function bindEvents(){
  ...
  elBtnShuffle?.addEventListener("click", ()=>{
    if (!items.length) return;
    shuffle(items); drawWheel(); saveState();
  });

  elBtnClear?.addEventListener("click", ()=>{
    items=[]; elInput.value=""; updateCount(); drawWheel(); saveState();
  });

  elBtnSpin?.addEventListener("click", ()=>{
    if (!items.length && elInput.value.trim()){
      let list=parseTextarea(elInput.value);
      if (elChkDedup.checked) list=dedup(list);
      items=list; updateCount(); drawWheel(); saveState();
    }
